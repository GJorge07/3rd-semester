...
int gbv_create(const char *filename, const char *key) {
    ...
    if (file) {
        ...
        fwrite(key, sizeof(char), 4, file);    /*escreve a chave primeiro (bytes 0-3)*/
        sb.num_docs = 0;                        /*inicia com 0 docs*/
        sb.offset_dir = 4 + sizeof(SuperBlock);     /*define onde começa dir (depois da chave + superbloco)*/
        fwrite(&sb,sizeof(SuperBlock),1,file); /*escreve superbloco do arquivo*/
        fclose(file);
        return 0;
...
int gbv_open(Library *lib, const char *filename) { 
    ...
    /* lê a chave (primeiros 4 bytes) */
    if (fread(key, sizeof(char), 4, file) != 4) {
        fclose(file);
        return 1;
    }

    /* lê o SuperBlock (após a chave) */
...
int gbv_add(Library *lib, const char *archive, const char *docname) {
    ...
    /* lê a chave (primeiros 4 bytes) */
    if(!fread(key, sizeof(char), 4, filee)) {
        fclose(file);
        fclose(filee);
        return 1;
    }
    ...
    /* escreve a chave (nos primeiros 4 bytes) */
    fseek(filee, 0, SEEK_SET);
    fwrite(key, sizeof(char), 4, filee);
    
    /* escreve o SuperBlock (após a chave) */
...
int gbv_remove(Library *lib, const char *docname, const char *filename) {
    ...
    for(i = 0; i < lib->count; i++) {     /*percorre os documentos*/
        ...
        if(strcmp(lib->docs[i].name,docname) == 0) {      /*se achar o documento, remove ele movendo todos pra esq*/
            ...
            /* lê a chave (primeiros 4 bytes) */
            if(!fread(key, sizeof(char), 4, file)) {
                fclose(file);
                return 1;
            }
